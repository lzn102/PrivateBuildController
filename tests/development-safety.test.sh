...
grep -Fq -- '--expires-in 3600' "$repo_root/scripts/deploy-development.sh"
grep -Fq -- '--continue-at -' "$repo_root/scripts/deploy-development.sh"
grep -Fq 'for attempt in 1 2 3 4 5 6' "$repo_root/scripts/deploy-development.sh"
grep -Fq 'Relay download failed after resumable attempts' "$repo_root/scripts/deploy-development.sh"
resume_line="$(grep -n -- '--continue-at -' "$repo_root/scripts/deploy-development.sh" | cut -d: -f1)"
checksum_line="$(grep -n '"$relay_sha256" "$archive" | sha256sum -c - >/dev/null$' "$repo_root/scripts/deploy-development.sh" | tail -1 | cut -d: -f1)"
test "$resume_line" -lt "$checksum_line"

...
for command_name in aws docker openssl sha256sum timeout zstd; do
...
object_key="${R2_OBJECT_KEY:-relay/${GITHUB_RUN_ID:-manual}-${GITHUB_RUN_ATTEMPT:-1}-$(openssl rand -hex 16).bin}"
[[ "$object_key" =~ ^relay/[A-Za-z0-9._-]+$ ]]
relay_passphrase="$(printf '%s' "$RELAY_ENCRYPTION_KEY:$BUILD_ID:$object_key" | sha256sum | cut -d ' ' -f 1)"

# Publish the cleanup handle before any operation can create a partial relay object.
umask 077
{
...
for attempt in 1 2 3; do
  if timeout --signal=TERM --kill-after=30s 20m \
      docker build --quiet --tag "$IMAGE_REF" "$SOURCE_DIR" >"$log" 2>&1; then
...
timeout --signal=TERM --kill-after=30s 20m aws s3 cp --only-show-errors --endpoint-url "$R2_ENDPOINT" \
